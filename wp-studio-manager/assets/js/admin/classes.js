        ...
        document.querySelectorAll('.gm-remove-athlete').forEach(btn => {
            btn.addEventListener('click', function(){
                ...
                fetch(adminPost, {
                    ...
                    body: 'action=wsm_remove_athlete&session_id=' + classId + '&athlete_id=' + athleteId
        ...
        document.querySelectorAll('.gm-remove-coach').forEach(btn => {
            btn.addEventListener('click', function(){
                ...
                fetch(adminPost, {
                    method: 'POST',
                    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                    body: 'action=wsm_remove_instructor&session_id=' + classId + '&coach_id=' + coachId
                }).then(r => r.text()).then(data => {
        ...
        document.querySelectorAll('.gm-edit-class-btn').forEach(btn => {
            btn.addEventListener('click', function(){
                ...
                fetch(adminAjax, {
                    method: 'POST',
                    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                    body: 'action=wsm_get_session_details&session_id=' + classId
        ...
        document.getElementById('gm-delete-class-btn').addEventListener('click', function(){
            ...
            if(confirm('Are you sure you want to delete this class?')){
                fetch(adminPost, {
                    method: 'POST',
                    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                    body: 'action=wsm_delete_session&session_id=' + classId + '&_wpnonce=' + deleteNonce
                }).then(r => r.text()).then(data => {
